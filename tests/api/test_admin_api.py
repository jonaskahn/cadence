...
class TestGlobalSettingsEndpoints:
    ...
    def test_update_response_contains_overridable_field(
        self, client: TestClient
    ) -> None:
        """PATCH /api/admin/settings/{key} response body includes the overridable field."""
        response = client.patch(
            f"{ADMIN_SETTINGS_URL}/max_tokens", json={"value": 8192}
        )
    ...
    def test_update_sends_overridable_true_to_service(
        self, client: TestClient, mock_settings_service: MagicMock
    ) -> None:
        """PATCH /api/admin/settings/{key} forwards overridable=True to the service."""
        client.patch(
            f"{ADMIN_SETTINGS_URL}/max_tokens",
            json={"value": 8192, "overridable": True},
        )

        call_kwargs = mock_settings_service.update_global_setting.call_args.kwargs
        assert call_kwargs["overridable"] is True

    def test_update_defaults_overridable_false_when_omitted(
        self, client: TestClient, mock_settings_service: MagicMock
    ) -> None:
        """PATCH /api/admin/settings/{key} defaults overridable to False when not in body."""
        client.patch(f"{ADMIN_SETTINGS_URL}/max_tokens", json={"value": 8192})
