...
# ---------------------------------------------------------------------------
# _prepare_context — context limit enforcement
# ---------------------------------------------------------------------------


def _make_settings_mock(
    max_context_window: int = 0,
    message_context_window: int = 0,
    enabled_auto_compact: bool = False,
) -> MagicMock:
    """Build a settings mock with real integer fields for context checks."""
    s = MagicMock()
    s.max_context_window = max_context_window
    s.message_context_window = message_context_window
    s.enabled_auto_compact = enabled_auto_compact
    return s


def _make_orchestrator_with_settings(settings: MagicMock) -> MagicMock:
    from cadence_sdk.types.sdk_messages import UvAIMessage

    orch = MagicMock()
    orch.mode_config = MagicMock()
    orch.mode_config.settings = settings
    orch.ask = AsyncMock(
        return_value={
            "messages": [UvAIMessage(content="reply")],
            "agent_hops": 0,
    ...
    orch.compact_history = AsyncMock(return_value="Previous conversation summary text.")
    return orch


class TestContextLimitEnforcement:
    """Tests for OrchestratorService._prepare_context context limit checks."""

    async def test_raises_when_token_limit_exceeded_and_autocompact_disabled(
        self, conversation_service: MagicMock, orchestrator_pool: MagicMock
    ) -> None:
        """process_chat raises RuntimeError when token count exceeds max_context_window and autocompact is off."""
        from cadence_sdk.types.sdk_messages import UvHumanMessage

        settings = _make_settings_mock(max_context_window=5, enabled_auto_compact=False)
        orch = _make_orchestrator_with_settings(settings)
        orchestrator_pool.get = AsyncMock(return_value=orch)
        # Return a big message that will exceed 5 token budget (4 chars/token)
        conversation_service.get_history = AsyncMock(
            return_value=[UvHumanMessage(content="x" * 100)]
        )
        service = OrchestratorService(
            pool=orchestrator_pool, conversation_service=conversation_service
        )

        with pytest.raises(RuntimeError, match="token count exceeded"):
            await service.process_chat("org", "inst", "u1", "hello")

    async def test_raises_when_message_count_exceeded_and_autocompact_disabled(
        self, conversation_service: MagicMock, orchestrator_pool: MagicMock
    ) -> None:
        """process_chat raises RuntimeError when history length exceeds message_context_window and autocompact is off."""
        from cadence_sdk.types.sdk_messages import UvHumanMessage

        settings = _make_settings_mock(
            message_context_window=2, enabled_auto_compact=False
        )
        orch = _make_orchestrator_with_settings(settings)
        orchestrator_pool.get = AsyncMock(return_value=orch)
        conversation_service.get_history = AsyncMock(
            return_value=[UvHumanMessage(content="msg")] * 5  # 5 > 2
        )
        service = OrchestratorService(
            pool=orchestrator_pool, conversation_service=conversation_service
        )

        with pytest.raises(RuntimeError, match="message count exceeded"):
            await service.process_chat("org", "inst", "u1", "hello")

    async def test_compact_called_when_token_limit_exceeded_and_autocompact_enabled(
        self, conversation_service: MagicMock, orchestrator_pool: MagicMock
    ) -> None:
        """process_chat calls compact_conversation when token limit exceeded and autocompact is on."""
        from cadence_sdk.types.sdk_messages import UvHumanMessage

        settings = _make_settings_mock(max_context_window=5, enabled_auto_compact=True)
        orch = _make_orchestrator_with_settings(settings)
        orchestrator_pool.get = AsyncMock(return_value=orch)
        conversation_service.get_history = AsyncMock(
            return_value=[UvHumanMessage(content="x" * 100)]
        )
        conversation_service.compact_conversation = AsyncMock()
        service = OrchestratorService(
            pool=orchestrator_pool, conversation_service=conversation_service
        )
        ...
        conversation_service.compact_conversation.assert_awaited_once()
        orch.compact_history.assert_awaited_once()

    async def test_compact_called_when_message_count_exceeded_and_autocompact_enabled(
        self, conversation_service: MagicMock, orchestrator_pool: MagicMock
    ) -> None:
        """process_chat calls compact_conversation when message count exceeded and autocompact is on."""
        from cadence_sdk.types.sdk_messages import UvHumanMessage

        settings = _make_settings_mock(
            message_context_window=2, enabled_auto_compact=True
        )
        orch = _make_orchestrator_with_settings(settings)
        orchestrator_pool.get = AsyncMock(return_value=orch)
        conversation_service.get_history = AsyncMock(
    ...
    async def test_no_error_when_within_both_limits(
        self, conversation_service: MagicMock, orchestrator_pool: MagicMock
    ) -> None:
        """process_chat proceeds normally when within both token and message limits."""
        from cadence_sdk.types.sdk_messages import UvHumanMessage

        settings = _make_settings_mock(
            max_context_window=10_000,
            message_context_window=50,
            enabled_auto_compact=False,
        )
        orch = _make_orchestrator_with_settings(settings)
        orchestrator_pool.get = AsyncMock(return_value=orch)
        conversation_service.get_history = AsyncMock(
            return_value=[UvHumanMessage(content="short")]
        )
        service = OrchestratorService(
            pool=orchestrator_pool, conversation_service=conversation_service
        )

        result = await service.process_chat("org", "inst", "u1", "hello")
        assert "conversation_id" in result
