...
Verifies that the factory correctly initialises its registry with all 21
...
from cadence.engine.factory import BackendRegistryEntry, OrchestratorFactory
...
class TestRegistryInitialization:
    ...
    def test_registers_all_langgraph_modes(self, factory: OrchestratorFactory) -> None:
        """Factory registers all 7 modes for LangGraph."""
        for mode in self._ALL_MODES:
            assert ("langgraph", mode) in factory.registry, f"langgraph/{mode} missing"

    def test_registers_all_openai_agents_modes(
        self, factory: OrchestratorFactory
    ) -> None:
        """Factory registers all 7 modes for openai_agents."""
        for mode in self._ALL_MODES:
            assert (
                "openai_agents",
                mode,
            ) in factory.registry, f"openai_agents/{mode} missing"

    def test_registers_all_google_adk_modes(self, factory: OrchestratorFactory) -> None:
        """Factory registers all 7 modes for google_adk."""
        for mode in self._ALL_MODES:
            assert (
                "google_adk",
                mode,
            ) in factory.registry, f"google_adk/{mode} missing"

    def test_total_combinations_is_twentyone(
        self, factory: OrchestratorFactory
    ) -> None:
        """Factory initializes exactly 21 (framework, mode) registry entries (3 frameworks × 7 modes)."""
        assert len(factory.registry) == 21

    def test_every_entry_has_required_component_keys(
        self, factory: OrchestratorFactory
    ) -> None:
        """Every registry entry is a BackendRegistryEntry with all required classes."""
        for combination, components in factory.registry.items():
            assert isinstance(
                components, BackendRegistryEntry
            ), f"Expected BackendRegistryEntry for {combination}"
            assert (
                components.adapter_class is not None
            ), f"Missing adapter_class for {combination}"
            assert (
                components.orchestrator_class is not None
            ), f"Missing orchestrator_class for {combination}"
            assert (
                components.streaming_wrapper_class is not None
...
class TestListSupportedBackends:
    ...
    def test_each_framework_exposes_seven_modes(
        self, factory: OrchestratorFactory
    ) -> None:
        """list_supported_backends lists exactly seven modes per framework."""
        supported = factory.list_supported_backends()

        for framework, modes in supported.items():
            assert len(modes) == 7, f"{framework} should have 7 modes"

    def test_langgraph_exposes_all_seven_modes(
        self, factory: OrchestratorFactory
    ) -> None:
        """list_supported_backends includes all 7 modes for LangGraph."""
        ...
        for mode in (
            ...
            assert mode in langgraph_modes, f"langgraph missing mode: {mode}"
