...
class SettingsService(OrchestratorConfigMixin):
    ...
    Attributes:
        ...
        org_repo: Organization repository (for tier lookup)
    ...
    async def set_global_setting(
        ...
        Args:
            ...
            value_type: Type of the value (string, int, bool, json, etc.)
        """
        self.logger.info(f"Setting global setting: {key}")
    ...
    async def update_global_setting(
        self, key: str, value: SettingValue, overridable: bool = False
        ...
        Returns:
            Updated setting dict or None if not found
        """
        existing = await self.global_settings_repo.get_by_key(key)
        if not existing:
            return None
        await self.set_global_setting(
            key, value, existing.description, overridable, existing.value_type
        )
        updated = await self.global_settings_repo.get_by_key(key)
        return {
            "key": updated.key,
            "value": updated.value,
    ...
    async def list_tier_definitions(self) -> list:
        """List all subscription tier definitions from global settings.

        Returns:
            List of tier definition dicts with key, tier_name, and quota,
            sorted by canonical tier rank (free → enterprise).
        """
        tiers = await self.global_settings_repo.get_by_category("tier")
        result = []
        for tier_setting in tiers:
            tier_name = tier_setting.key.removeprefix("tier.")
            result.append(
                {
                    "key": tier_setting.key,
                    "tier_name": tier_name,
                    "quota": tier_setting.value,
                }
            )
        result.sort(
            key=lambda d: (
                SUBSCRIPTION_TIER_SORT_ORDER.index(d["tier_name"])
                if d["tier_name"] in SUBSCRIPTION_TIER_SORT_ORDER
                else len(SUBSCRIPTION_TIER_SORT_ORDER)
            )
        )
        return result

    async def get_org_quota(self, org_id: str) -> Optional[dict[str, Any]]:
        """Get the quota limits for an organization based on its subscription tier.

        Args:
            org_id: Organization identifier

        Returns:
            Quota dict from the tier definition, or None if not found
        """
        ...
        if not org:
            return None
        tier_name = getattr(org, "subscription_tier", "free") or "free"
        tier_key = f"tier.{tier_name}"
        setting = await self.global_settings_repo.get_by_key(tier_key)
        return setting.value if setting else None
    ...
    async def delete_global_setting(self, key: str) -> None:
        ...
        Args:
            key: Setting key
        """
        self.logger.info(f"Deleting global setting: {key}")
        await self.global_settings_repo.delete(key)

    async def resolve_effective_setting(
        self, key: str, org_id: str, instance_config: dict | None = None
    ) -> Any:
        """Resolve the effective value for a key across all tiers.

        Args:
            key: Setting key
            org_id: Organization ID
            instance_config: Optional instance config JSONB blob

        Returns:
            Effective value after applying cascade rules, or None if key not found
        """
    ...
    async def set_tenant_setting(
        ...
        Args:
            ...
            value: Setting value
        """
        self.logger.info(f"Setting tenant setting: {org_id}/{key}")
    ...
    async def delete_tenant_setting(self, org_id: str, key: str) -> None:
        ...
        Args:
            ...
            key: Setting key
        """
        self.logger.info(f"Deleting tenant setting: {org_id}/{key}")
    ...
    async def delete_instance(self, instance_id: str) -> None:
        ...
        Args:
            instance_id: Instance ID
        """
        self.logger.info(f"Soft-deleting instance: {instance_id}")
    ...
    async def update_instance_status(
        ...
        """
        self.logger.info(f"Updating instance {instance_id} status to {status}")
    ...
    async def update_instance_config(
        ...
        self.logger.info(f"Updating instance config: {instance_id}")
    ...
    async def update_orchestrator_metadata(
        ...
        Args:
            ...
            default_llm_config_id: New default LLM config ID to merge into config JSONB
            caller_id: User ID performing the operation
        ...
        Raises:
            ValueError: If instance not found, deleted, or wrong org
        """
        instance = await self.instance_repo.get_by_id(instance_id)
        if not instance:
        ...
        updated = await self.instance_repo.update_metadata(
            instance_id=instance_id,
            name=name,
            tier=tier,
