...
class SharedBundleCache(BaseSharedCache):
    ...
    async def get_or_create(
        ...
        """
        if not is_stateless:
            self.logger.debug(f"Plugin {plugin_pid} is stateful, not caching")
        ...
        async with self._lock:
            ...
            self.logger.info(f"Creating new bundle {plugin_pid}@{version}")
            bundle = await bundle_factory()
    ...
    async def decrement_ref(
        ...
        async with self._lock:
            ...
            if new_count <= 0:
                self.logger.info(f"Removing bundle {plugin_pid} (ref_count=0)")
                ...
                self.logger.debug(
                    f"Decremented ref for {plugin_pid}, ref_count={new_count}"
                )
            ...
            for bundle, _ in self._registry.values():
                if hasattr(bundle, "cleanup"):
                    try:
                        ...
                        self.logger.error(f"Bundle cleanup error: {e}")

            self._registry.clear()
            self.logger.info(f"Cleaned up {count} bundles from cache")
