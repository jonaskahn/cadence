...
class SharedTemplateCache(BaseSharedCache):
    ...
    async def get_or_create(
        ...
        if framework_type != LANGGRAPH_FRAMEWORK:
            self.logger.debug(
                f"Template caching only for LangGraph, got {framework_type}"
            )
    ...
    async def decrement_ref(
        ...
        async with self._lock:
            ...
            if new_count <= 0:
                self.logger.info(
                    f"Removing template {framework_type}/{mode} (ref_count=0)"
                )
    ...
    async def cleanup(self) -> None:
        ...
        async with self._lock:
            ...
            self.logger.info(f"Cleaned up {count} templates from cache")
