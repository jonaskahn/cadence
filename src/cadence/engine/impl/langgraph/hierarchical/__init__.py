"""LangGraph hierarchical orchestrator — not yet implemented."""
...
from cadence.engine.modes import HierarchicalMode
from cadence.infrastructure.plugins import SDKPluginManager

logger = logging.getLogger(__name__)


class LangGraphHierarchical(BaseLangGraphOrchestrator):
    """Placeholder hierarchical orchestrator for LangGraph. Not yet implemented."""
    ...
    def __init__(
        ...
        self.mode_config = HierarchicalMode(
            Framework.LANGGRAPH, resolved_config.get("mode_config", {})
        )

    async def _build_resources(self) -> None:
        self.logger.warning("LangGraph hierarchical is not yet implemented")

    async def astream(self, state: UvState) -> AsyncIterator[StreamEvent]:
        raise NotImplementedError("LangGraph hierarchical is not yet implemented")
