"""OpenAI Agents SDK pipeline orchestrator — not yet implemented."""
...
from cadence.engine.modes import PipelineMode
from cadence.infrastructure.plugins import SDKPluginManager

logger = logging.getLogger(__name__)


class OpenAIPipeline(BaseOrchestrator):
    """Placeholder pipeline orchestrator for OpenAI Agents SDK. Not yet implemented."""
    ...
    def __init__(
        ...
        self.mode_config = PipelineMode(
            "openai_agents", resolved_config.get("mode_config", {})
        )

    async def _build_resources(self) -> None:
        self.logger.warning("OpenAI Agents SDK pipeline is not yet implemented")

    async def astream(self, state: UvState) -> AsyncIterator[StreamEvent]:
        raise NotImplementedError("OpenAI Agents SDK pipeline is not yet implemented")
