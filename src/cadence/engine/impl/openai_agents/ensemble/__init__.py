"""OpenAI Agents SDK ensemble orchestrator — not yet implemented."""
...
from cadence.engine.modes import EnsembleMode
from cadence.infrastructure.plugins import SDKPluginManager

logger = logging.getLogger(__name__)


class OpenAIEnsemble(BaseOrchestrator):
    """Placeholder ensemble orchestrator for OpenAI Agents SDK. Not yet implemented."""
    ...
    def __init__(
        ...
        self.mode_config = EnsembleMode(
            "openai_agents", resolved_config.get("mode_config", {})
        )

    async def _build_resources(self) -> None:
        self.logger.warning("OpenAI Agents SDK ensemble is not yet implemented")

    async def astream(self, state: UvState) -> AsyncIterator[StreamEvent]:
        raise NotImplementedError("OpenAI Agents SDK ensemble is not yet implemented")
