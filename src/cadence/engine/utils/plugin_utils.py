...
def build_routing_prompt(plugin_descriptions: List[str]) -> str:
    ...
    for plugin_index, plugin_description in enumerate(plugin_descriptions, 1):
        prompt += f"{plugin_index}. {plugin_description}\n\n"
...
def build_all_plugins_description(bundles: Dict[str, SDKPluginBundle]) -> str:
    ...
    for pid, bundle in bundles.items():
        plugin_metadata = bundle.metadata
        capabilities = (
            ", ".join(plugin_metadata.capabilities)
            if plugin_metadata.capabilities
            else "general"
        )
        tool_names = ", ".join(t.name for t in bundle.orchestrator_tools)
        parts.append(
            f"Plugin: {plugin_metadata.name} (pid={pid})\n"
            f"Description: {plugin_metadata.description}\n"
