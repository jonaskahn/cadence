...
class EmbeddingService(Loggable):
    ...
    def _get_cached_embedding(self, text: str) -> Optional[List[float]]:
        ...
        if cache_key in self._cache:
            self.logger.debug(f"Embedding cache hit for: {text[:50]}...")
    ...
    async def _generate_openai_embedding(self, text: str) -> List[float]:
        ...
        """
        try:
            self.logger.debug(f"Generating OpenAI embedding for: {text[:50]}...")
            return self._generate_mock_embedding(text)

        except Exception as error:
            self.logger.error(
                f"OpenAI embedding generation failed: {error}", exc_info=True
            )
            raise
    ...
    async def _generate_azure_embedding(self, text: str) -> List[float]:
        ...
        """
        try:
            self.logger.debug(f"Generating Azure embedding for: {text[:50]}...")
            return self._generate_mock_embedding(text)

        except Exception as error:
            self.logger.error(
                f"Azure embedding generation failed: {error}", exc_info=True
            )
            raise
    ...
    async def _generate_voyage_embedding(self, text: str) -> List[float]:
        ...
        """
        try:
            self.logger.debug(f"Generating Voyage embedding for: {text[:50]}...")
            return self._generate_mock_embedding(text)

        except Exception as error:
            self.logger.error(
                f"Voyage embedding generation failed: {error}", exc_info=True
            )
            raise
    ...
    async def _generate_cohere_embedding(self, text: str) -> List[float]:
        ...
        """
        try:
            self.logger.debug(f"Generating Cohere embedding for: {text[:50]}...")
            return self._generate_mock_embedding(text)

        except Exception as error:
            self.logger.error(
                f"Cohere embedding generation failed: {error}", exc_info=True
            )
            raise
