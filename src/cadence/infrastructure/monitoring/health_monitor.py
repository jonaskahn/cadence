...
class HealthMonitor(Loggable):
    ...
    async def _monitor_loop(self) -> None:
        ...
        self.logger.info("HealthMonitor loop started")
        ...
        while self._running:
            try:
                ...
                self.logger.info("HealthMonitor loop cancelled")
                break

            except Exception as error:
                self.logger.error(f"HealthMonitor loop error: {error}", exc_info=True)
                await asyncio.sleep(self.check_interval)
    ...
    async def _run_health_checks(self) -> None:
        ...
        try:
            ...
            if not instance_ids:
                self.logger.debug("No orchestrators to check")
                return

            self.logger.debug(
                f"Running health checks for {len(instance_ids)} instances"
            )
            ...
            self.logger.error(f"Health check batch failed: {error}", exc_info=True)
    ...
    async def _check_instance_health(self, instance_id: str) -> None:
        ...
        try:
            ...
            if health_status.get("is_ready", False):
                ...
                self.logger.debug(f"Health check PASS: {instance_id}")
            else:
            ...
            self.logger.error(
                f"Health check ERROR: {instance_id}, error={error}",
                exc_info=True,
            )
    ...
    async def _handle_health_check_failure(
        ...
        self._record_failure(instance_id)
        self.logger.warning(
            f"Health check FAIL: {instance_id}, " f"status={health_status}"
        )
    ...
    def _record_success(self, instance_id: str) -> None:
        ...
        if instance_id in self._recovery_attempts:
            self.logger.info(f"Instance recovered: {instance_id}")
    ...
    async def _attempt_recovery(self, instance_id: str) -> None:
        ...
        Args:
            instance_id: Instance ID
        """
        self.logger.info(f"Attempting recovery: {instance_id}")
        ...
        try:
            ...
            self.logger.error(
                f"Recovery attempt failed: {instance_id}, error={error}",
                exc_info=True,
            )
