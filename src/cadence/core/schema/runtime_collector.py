...
class RuntimeSchemaCollector(Loggable):
    ...
    def _create_additional_data_mapping(self, relevant_schemas: Dict[str, Type[TypedDict]]) -> Type[TypedDict]:
    ...
    def extract_plugin_data(self, structured_response: Dict[str, Any], plugin_name: str) -> List[Dict[str, Any]]:
        """Return list of items for a specific plugin from additional_data mapping."""
        additional_data = structured_response.get("additional_data", {}) or {}
        if isinstance(additional_data, dict):
            items = additional_data.get(plugin_name, [])
            return items if isinstance(items, list) else []
        return []

    def create_additional_data_schema(self, active_plugins: Optional[List[str]] = None) -> Type[TypedDict]:
        ...
        additional_data_td = self._create_additional_data_mapping(relevant_schemas)
