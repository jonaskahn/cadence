...
class OrchestratorInstanceRepository:
    ...
    async def update_metadata(
        ...
        """Update name, tier, and/or merge a config patch into the config JSONB.

        Args:
            instance_id: Instance identifier (UUID or str)
            name: New name or None to leave unchanged
            tier: New tier or None to leave unchanged
            config_patch: Dict to shallow-merge into config JSONB, or None
            caller_id: User ID performing the operation
        ...
        async with self.client.session() as session:
            ...
            if instance:
                ...
                if tier is not None:
                    instance.tier = tier
                if config_patch:
                    instance.config = {**(instance.config or {}), **config_patch}
                instance.updated_at = utc_now()
                instance.updated_by = caller_id
                await session.flush()
                return self._serialize(instance)
    ...
    async def count_using_llm_config(self, org_id: str | UUID, config_id: int) -> int:
        """Count active instances in the org that reference an LLM config by ID.

        Checks the top-level default_llm_config_id and any nested llm_config_id
        at any depth inside mode_config via JSONPath recursive descent — works
        for all modes (supervisor, coordinator, handoff) without hardcoding node names.

        Args:
            org_id: Organization identifier
            config_id: LLM config primary key to search for

        Returns:
            Number of active (non-deleted) instances that reference this config
        """
        if isinstance(org_id, str):
            org_id = UUID(org_id)
        config_id_string = str(config_id)
        jsonpath = f"$.**.llm_config_id ? (@ == {config_id})"
        async with self.client.session() as session:
            result = await session.execute(
                select(func.count()).where(
                    ...
                    ~OrchestratorInstance.is_deleted,
                    or_(
                        OrchestratorInstance.config["default_llm_config_id"].astext
                        == config_id_string,
                        func.jsonb_path_exists(OrchestratorInstance.config, jsonpath),
                    ),
                )
            )
