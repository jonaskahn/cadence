...
class GlobalSettingsRepository:
    ...
    async def get_by_category(self, category: str) -> List[GlobalSettings]:
        """Retrieve all non-deleted settings with the given category.

        Args:
            category: Category tag (e.g. 'orchestrator')
        ...
        async with self.client.session() as session:
            result = await session.execute(
                select(GlobalSettings).where(
                    GlobalSettings.category == category,
                    ~GlobalSettings.is_deleted,
                )
            )
            return list(result.scalars().all())
    ...
    async def upsert(
        ...
        async with self.client.session() as session:
            ...
            if existing:
                ...
                existing.overridable = overridable
